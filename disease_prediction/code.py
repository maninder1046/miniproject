...
for x in range(0,len(l1)):
    l2.append(0)                                                #all element of l2 becomes 0 till length of l1
                       ...
                       # TESTING DATA
test=pd.read_csv("https://raw.githubusercontent.com/maninder1046/miniproject/main/disease_prediction/testing.csv")              #reading csv file by pd
...
S1En = OptionMenu(maninderGUI, Symptom1,*OPTIONS)
S1En.grid(row=7, column=2)

S2En = OptionMenu(maninderGUI, Symptom2,*OPTIONS)
S2En.grid(row=8, column=2)

S3En = OptionMenu(maninderGUI, Symptom3,*OPTIONS)
S3En.grid(row=9, column=2)

S4En = OptionMenu(maninderGUI, Symptom4,*OPTIONS)
S4En.grid(row=10, column=2)

S5En = OptionMenu(maninderGUI, Symptom5,*OPTIONS)
S5En.grid(row=11, column=2)

NameLb = Label(maninderGUI, text="")
...
t3 = Text(maninderGUI, height=2, width=30,bg="grey")
t3.config(font=("Elephant", 20))
t3.grid(row=20, column=1 , padx=10)

maninderGUI.mainloop()                             #program runs till you close the window
